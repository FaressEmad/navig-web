...
export default function LandingPageClient({
  ...
  // Fast count-up to 500 on mount, then increment dynamically
  React.useEffect(() => {
    let start = 0;
    const end = 500;
    const duration = 1000; // 1 second rollup
    const steps = 50;
    const incrementStep = end / steps;
    ...
    const timer = setInterval(() => {
      ...
      if (start >= end) {
        ...
        // Periodically tick upwards to simulate active navigation events
  ...
  // Ensure fallback default numbers if database is empty/seeding
  const displayFaculties = totalFaculties > 0 ? totalFaculties : 14;
  const displayBuildings = totalBuildings > 0 ? totalBuildings : 11;
  const displayReferences = totalReferences > 0 ? totalReferences : 54;
          ...
          <div className="md:col-span-7 text-center md:text-left flex flex-col items-center md:items-start">
            ...
            <div className="mt-12 flex items-center gap-3">
              <div className="flex -space-x-2">
                <div className="w-9 h-9 rounded-full border-2 border-white bg-slate-200 animate-pulse" />
              ...
              <p className="text-xs font-semibold text-secondary min-w-[240px] text-left">
                {language === "ar" 
                  ? `أكثر من ${studentCount.toLocaleString("ar-EG")}+ طالب وطالبة بيتحركوا بيه في الجامعة` 
                  : `${studentCount.toLocaleString("en-US")}+ Students using CU Navigate`
                }
              </p>
            </div>
          </div>
      ...
      <section className="py-16 bg-surface-container/30 px-margin-mobile md:px-margin-desktop border-y border-outline-variant/5">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 md:gap-12">
            ...
            <div className="text-center">
              <span className="block text-4xl md:text-5xl font-black text-primary mb-2">{displayBuildings}</span>
              <p className="text-xs md:text-sm font-semibold text-secondary max-w-[160px] mx-auto">{t("stats2Lbl")}</p>
            </div>
            <div className="text-center">
              <span className="block text-4xl md:text-5xl font-black text-primary mb-2">{displayReferences}</span>
